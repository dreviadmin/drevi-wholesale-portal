...
- [ ] ANSH-19 · Per-design Drive folders · consolidate each SKU's photos, clear `docs/DRIVE-FOLDER-AUDIT.md` (188-design checklist already generated), then supply `DRIVE_DESIGN_FOLDER_ID` · unblocks every image-upload path (§4.1). Until set: uploads disabled with an inline message, everything else works. **IMPORTANT — make the parent a Shared Drive folder (or grant the service account Manager on it):** a service account has no personal Drive quota, so uploads into a My Drive folder fail with `storageQuotaExceeded` (we hit exactly this when copying the registry sheet on 25 Jul). Folder *matching* and all reads work either way.
